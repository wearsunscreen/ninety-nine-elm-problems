# Problem 38


Use the solutions of problems 34 and 37 to compare the algorithms. Measure the time for each to solve phi of 10090.

Below is a test application to measure the relative performance of the two solutions. It uses the Elm Architecture to be able to get the time. 

```elm
import Html exposing (div, button, p, text)
import Html.App as App
import Html.Events exposing (onClick)
import Task
import Time exposing (Time)


{- ======================================================
    Test 1
-}

test1 : Int -> Int
test1 n =
    test totient n
   
    
totient :  Int -> Int
totient n = 
    List.length <| List.filter (\x -> coprime n x) [1..n]


coprime :  Int -> Int -> Bool
coprime a b = 
    gcd a b == 1


gcd : Int -> Int -> Int 
gcd a b =
    if b == 0 then  
        abs a
    else
        gcd b (a % b)


{- ======================================================
    Test 2
-}


test2 : Int -> Int
test2 n =
    test totient n       
    

{- ======================================================
    Timing test app
-}


test : (Int -> Int) -> Int -> Int
test f n =
    List.length
        <|List.map f [1..n]

main =
    App.program
        { init = init
        , view = view
        , update = update
        , subscriptions = (\_ -> Sub.none)
        }
 
 
view model =
    div []
        [ button [ onClick (ExecuteTest 1)] [ text "Test 1" ]
        , div []
          [ text <| case fst model.testTimes1 of
              Err msg -> msg
              Ok s -> case snd model.testTimes1 of
                  Err msg -> msg
                  Ok e -> toString (e - s)
          , p [] []
          , text <| case fst model.testTimes2 of
              Err msg -> msg
              Ok s -> case snd model.testTimes2 of
                  Err msg -> msg
                  Ok e -> toString (e - s)
          ]
        , button [ onClick (ExecuteTest 2)] [ text "Test 2" ]
        ]
 
 
type Msg
    = NoOp
    | ExecuteTest Int
    | GetTimeFailure String
    | StartTime Int Time
    | RunTest Int Time
    | EndTest Int Time
   

type alias TestTimes = (TestTime, TestTime)
type alias TestTime = Result String Time
 
type alias Model =
    { testReps : Int -- don't hard code this or Elm will memoize the test functions
    , testTimes1 : TestTimes
    , testTimes2 : TestTimes
    }
 
init : (Model , Cmd Msg)
init =
  ( { testReps = 1000
    , testTimes1 = (Err "Please Run Test 1", Err "No end time yet!")
    , testTimes2 = (Err "Please Run Test 2", Err "No end time yet!")
    }
  , Cmd.none
  )
 
 
update : Msg -> Model -> ( Model, Cmd Msg )
update msg model =
    case msg of
        EndTest n time ->
            ( case n of
                1 -> 
                    { model | testTimes1 = (fst model.testTimes1, Ok time) }
                2 -> 
                    { model | testTimes2 = (fst model.testTimes2, Ok time) }
                _ -> 
                    model
            , Cmd.none
            )
 
        ExecuteTest n ->
            ( model
            , Task.perform  GetTimeFailure (StartTime n) Time.now
            )
 
        GetTimeFailure msg ->
            ( { model | testTimes1 = (Err "No start time yet!", Err "No end time yet!")}
            , Cmd.none
            )
            
        NoOp -> (model, Cmd.none)
 
        RunTest n time ->
            ( case n of
                1 -> 
                    { model 
                    | testTimes1 = (Ok time, Err "No end time yet!") 
                    , testReps = always model.testReps (test1 model.testReps)
                    }
                2 -> 
                    { model
                    | testTimes2 = (Ok time, Err "No end time yet!") 
                    , testReps = always model.testReps (test2 model.testReps)
                    }
                _ -> 
                    model
            , Task.perform  GetTimeFailure  (EndTest n) Time.now
            )
 
        StartTime n time ->
            ( model
            , Task.perform  GetTimeFailure (RunTest n) Time.now
            )
 
```


